...
for idx, kind in enumerate(['qiskit', 'qssa_updated']):
    ratio = lambda p: 100*(1-p.getKind(kind).tot / p.getKind('default').tot)
    col = ''
    if kind == 'qiskit':
        col = light_gray
        label = 'qiskit'
    if kind == 'qssa_updated':
        col = dark_blue
        label = 'qssa'
    rects1 = ax.bar(xs - width/2 + (idx*1 * width), [ratio(p) for p in to_plot], width, label=label, color=col)
...
fig.tight_layout()
fig.savefig('optim-ratio-new.pdf')
...
fig.set_size_inches(5,2)
fig.tight_layout()
fig.savefig('opt-fact-per-unit-time-new.pdf')
