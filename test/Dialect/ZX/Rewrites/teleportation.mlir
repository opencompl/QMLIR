...
func @CNOT(%a0 : !zx.wire, %b0 : !zx.wire) -> (!zx.wire, !zx.wire) {
  ...
  %a1, %m = zx.Z(%zero : f32) %a0 : !zx.wire, !zx.wire
  %b1 = zx.X(%zero : f32) %m, %b0 : !zx.wire
...
func @teleport() {
  ...
  // CNOT a, b
  %a1, %m0 = zx.Z(%zero : f32) %a0 : !zx.wire, !zx.wire
  %b1 = zx.X(%zero : f32) %m0, %b0 : !zx.wire

  %a2 = zx.H %a1

  // CNOT b, c
  %b2, %m1 = zx.Z(%zero : f32) %b1 : !zx.wire, !zx.wire
  %c1 = zx.X(%zero : f32) %m1, %c0 : !zx.wire

  %c2 = zx.H %c1

  // CNOT a, c
  %a3, %m2 = zx.Z(%zero : f32) %a2 : !zx.wire, !zx.wire
  %c3 = zx.X(%zero : f32) %c2, %m2 : !zx.wire
